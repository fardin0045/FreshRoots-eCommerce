...
const ImageUpload = ({productData,setProductData}) => {
    ...
    const removeImage = (index)=>{
        setProductData((prev)=>{
            const updatedImage = prev.productImg.filter((_, i)=>i !== index);
                ...
                <div className="grid grid-cols-2 gap-4 mt-3 sm:grid-cols-3">
                    {
                        productData.productImg.map((file,index)=>{
                            //check if file is already a file or db object
                            let preview;
                            ...
                            return(
                                <Card key={index} className="relative group overflow-hidden ">
                                    <CardContent>
                                        <img src={preview} width={200} height={200} className="w-full h-32 object-cover rounded-md" alt="" />
                                        <button onClick={()=>removeImage(index)}className="absolute top-1 bg-black/50 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition" > <X size={14}/> </button>
                                    </CardContent>
                                </Card>
                            )
                        })
